...
class Orders
{
    ...
    public const STATUT_CANCELLED = 'annulée';
     ...
     */
    #[ORM\OneToMany(targetEntity: OrderLines::class, mappedBy: 'orders', cascade: ['persist'], orphanRemoval: true)]
    ...
    public function addProduct(Products $product, int $quantity = 1): void
    {
    ...
    public function removeProduct(Products $product, int $quantity = 1) 
    {
        $quantity = max(1, $quantity);
        foreach ($this->orderLines as $line) {
            if ($line->getProduct() === $product) {
